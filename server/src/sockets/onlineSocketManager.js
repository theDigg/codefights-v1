const io = require("../server").ioOnline;
...
module.exports.ioOnline = socket => {
  socket.on("GET_CONNECTED_USERS", () => {
    Object.keys(connectedUsers).forEach(async username => {
      connectedUsers[username] = await getUser(username);
    });
